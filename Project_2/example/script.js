...
function draw(error, data) {
    ...
    // what are the min and maximum temperatures?
    var maxHighTemp = d3.max(weekWeather, function (d) {
        return d.temperatureHigh;
    });
    var minLowTemp = d3.min(weekWeather, function (d) {
    ...
    plotLines
        ...
        .attr("x1", function (d) {
            return scaleX2(new Date(d.time * 1000));
        })
        .attr("x2", function (d) {
            return scaleX2(new Date(d.time * 1000));
        })
        .attr("y1", function (d) {
            return scaleY2(d.temperatureHigh);
        })
        .attr("y2", function (d) {
            return scaleY2(d.temperatureMin);
        });
