...
plot1.select('.bars')
    ...
    .attr("x", function (d) {
        return scaleXPlot1(d.year);
    })
    .attr("y", function (d) {
        return scaleYPlot1(d.fruit);
    })
    .attr("width", scaleXPlot1.bandwidth())
    .attr("height", function (d) {
        return (height1 - scaleYPlot1(d.fruit));
    })
    .style("fill", "#6922ef");
...
function drawData(dataAll) {
    ...
    // 1 map (get an array) all the years in the original array
    var mapYears = dataAll.map(function (d) {
        return d.year;
    });
    ...
    // 2 get the minimum and maximum values
    var maxVeg = d3.max(dataAll, function (d) {
        return d.vegetable;
    });
    ...
    var lineFruit = d3.line()
        .x(function (d) {
            return scaleXPlot2(d.year);
        })
        .y(function (d) {
            return scaleYPlot2(d.fruit);
        });
    ...
    var lineVeg = d3.line()
        ...
        .y(function (d) {
            return scaleYPlot2(d.vegetable);
        });
    ...
    plot2.append('g')
        ...
        .attr('class', 'axis axis-x');
    plot2.append('g')
        .attr('transform', 'translate(' + (margin2.l + width2 / (dataAll.length * 2)) + ',' + margin2.t + ')')
        .attr('class', 'produce');
