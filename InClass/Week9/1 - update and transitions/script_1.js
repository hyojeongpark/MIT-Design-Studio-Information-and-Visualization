...
var plotMedal = plot1.append('g')
    .attr('transform', 'translate(' + margin.l + ',' + margin.t + ')')
    .attr('class', 'medals');
var plotText = plot1.append('g')
    .attr('transform', 'translate(' + margin.l + ',' + margin.t + ')')
    .attr('class', 'text');
...
var scaleX = d3.scaleBand()
    .rangeRound([maxR * 2, (width - (maxR * 2))])
    .domain([1, 2, 3, 4, 5])
    .padding(0.5);
...
function dataloaded(err, data) {
    var extentMedals = d3.extent(data, function (d) {
        return d.gold
    });
    ...
    function draw(year) {
        ...
        text.enter().append("text")
            .attr("class", "countries").text(function (d) {
                return d.abv;
            })
